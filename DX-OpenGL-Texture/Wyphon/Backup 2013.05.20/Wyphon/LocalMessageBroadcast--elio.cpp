...
//		wcout << "ListenerThread function: " << *(pLocalMessageBroadcastPartner->myName) << "\n";
	 ...
	 *
	 * the first parameter needs to be a working handle
	 * the second one may be corrupted (this might be the reason why we need to remove it)
	 */
	extern "C" _declspec(dllexport)
	bool RemovePartnerFromSharedMemory(HANDLE localMessageBroadcastPartnerHandle, unsigned __int32 partnerIdToRemove) {
...
//		wcout << "Trying to DestroyLocalMessageBroadcastPartner " << pLocalMessageBroadcastPartner << "\n";
		...
		success = RemovePartnerFromSharedMemory(localMessageBroadcastPartnerHandle, pLocalMessageBroadcastPartner->myId);
...
//	}
	...
	extern "C" _declspec(dllexport)
	unsigned __int32 GetAllBroadcastPartnerIds(HANDLE localMessageBroadcastPartnerHandle, unsigned __int32* &ids) {
		BYTE * idBytes = NULL;
		unsigned __int32 nrOfIds = 0;
		try {
			LocalMessageBroadcastPartnerDescriptor * pLocalMessageBroadcastPartner = (LocalMessageBroadcastPartnerDescriptor *) localMessageBroadcastPartnerHandle;
			if ( LockSharedMemory( pLocalMessageBroadcastPartner->hSharedMemory, 2000 ) ) {
				nrOfIds = ReadSharedMemory( pLocalMessageBroadcastPartner->hSharedMemory, idBytes ) / sizeof(unsigned __int32);
			}
		}
		catch( ... ) {
		}
