...
namespace LocalMessageBroadcast {
	...
	void CleanupOtherPartner( void* pLocalMessageBroadcastPartnerHandle, unsigned __int32 partnerId );
...
//		    	wcout << "Try to write to mailsot " << hFile << " aka " << itr->second << "\n";
			...
			if (!fResult) {
				CleanupOtherPartner(pLocalMessageBroadcastPartner, partnerId);
				success = false;
...
//			wcout << "WHAT'S THIS??? dataSize = " << dataSize << " but addr - data = " << addr - (*data) << "\n";
	...
	///
	/// int32 forOtherPartnerId :	if set, the goodbye message will be sent for another partner
	///								(probably after realizing that it does not exist any more)
	///
	/// @author: frederik
	/// @author: elio (send goodbye for a partner that already faded away)
	__int32 CreateGoodbyeMessage(LocalMessageBroadcastPartnerDescriptor * pLocalMessageBroadcastPartner, BYTE ** data, unsigned __int32 forOtherPartnerId = NULL ) {
		unsigned __int32 partnerId = forOtherPartnerId ? forOtherPartnerId : pLocalMessageBroadcastPartner->myId;
		__int32 dataSize = 1 + sizeof(partnerId);
		(*data) = new BYTE[dataSize];
		(*data)[0] = LOCAL_MSG_BROADCAST_GOODBYE;

		BYTE * addr = (*data) + 1;
		__int32 len = sizeof(partnerId);
		CopyMemory( addr, (VOID *) &partnerId, len );
		addr += len;
...
//		wcout << "The new message partnerId = " << partnerId << " and name " << (*namesMap)[partnerId] <<  " (length=" << name_length << ")" << "\n";
	...
	void CleanupOtherPartner( void* pLocalMessageBroadcastPartnerHandle, unsigned __int32 partnerId ) {
		...
		__int32 dataSize = CreateGoodbyeMessage(pLocalMessageBroadcastPartner, &data, partnerId);
			// tell all other partners that he died
		WriteToAllMailslots( pLocalMessageBroadcastPartner, (BYTE *) data, dataSize );
			// also simulate "write to own mailslot", we want to cleanup our data as well
